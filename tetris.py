...
def draw_text_below_middle(surface, text, size, color):
    font = pygame.font.SysFont('times new roman', size, bold=True)
    label = font.render(text, 1, color)
...
def clear_rows(grid, locked):
    ...
    #making the new row at the top of the grid
    if inc > 0:
...
def main(win):
    ...
    while run:
        ...
        if check_lost(locked_positions):
            ...
            draw_text_below_middle(win, "Score:" + str(score), 30, (255,255,255))
            pygame.display.update() 
...
def main_menu(win):
    ...
    while run:
        ...
        draw_text_below_middle(win, 'Use arrow keys to move. Up key to rotate.', 20, (255,255,255))
        pygame.display.update()
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                run = False
